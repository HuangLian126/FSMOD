...
class RPNModule(torch.nn.Module):
    """
    Module for RPN computation. Takes feature maps from the backbone and outputs RPN proposals and losses. Works for both FPN and non-FPN.
    """
    ...
    def forward(self, images, features, targets=None):
        ...
        Arguments:
            ...
            features (list[Tensor]): features computed from the images that are used for computing the predictions.
    ...
    def _forward_train(self, anchors, objectness, rpn_box_regression, targets):
        if self.cfg.MODEL.RPN_ONLY:
            ...
            # When training an RPN-only model, the loss is determined by the predicted objectness and
            # rpn_box_regression values and there is no need to transform the anchors into predicted boxes;

            # this is an optimization that avoids the unnecessary transformation.
            boxes = anchors
        else:
